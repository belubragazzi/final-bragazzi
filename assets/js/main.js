...
Utilizando el array dado, utilizar Template Strings para producior la siguiente cadena. 
Asegurarse que los numeros, nombres y nombre del equipo, provengan de los datos
...
const encontrar = people.filter(({role}) => role === "senior");

const message = `Hay ${people.length} personas en el ${teamName} team. \n Sus nombres son ${people.map(({name})=> ' ' + name)}. \n Hay ${encontrar.length} de ellos son ${people[0].role} con experiencia`
...
const {instagram, twitter} = profile.social;
...
/*
 _3_c Utilizar map() y destructuring para obtener un array con la multiplicacion
  de ambos numeros en cada posición.
...
_4_a Obtener los semifinalistas (true) de cada grupo en dos arrays (Nombre: semiFinalistasZonaA, 
    Nombre: semiFinalistasZonaAB) utilizando filter(), luego unir todos los objetos 
    en otro array (Nombre: semiFinal) mediante Rest operator (...)
 
_4_b Obtener un array (Nombre: final) con los objetos de Argentina y Brasil. 
Utilizar if ternario y el operador || (or): 
//cond ? true (objeto) : false

_4_c Obtener un array finalResultado mediante final.map(), agregandole la clave goles 
utilizando Rest Parameters a cada objeto del array: 
goles: Math.floor(Math.random() * 101)

_4_d Modificar la constante campeon para que compare los goles de cada equipo 
y guarde una cadena que diga: 
...
const final = semiFinal.filter((semiFinal)  => semiFinal.pais === "Argentina" || semiFinal.pais === "Brasil" ? semiFinal : false);


/*
4_c Obtener un array finalResultado mediante final.map(), agregandole la clave goles 
utilizando Rest Parameters a cada objeto del array: 
goles: Math.floor(Math.random() * 101)
...
*/
/* 
const finalResultado = final.map((final) => [final, goles = Math.floor(Math.random() * 101)])  ;

const campeon = `El campeón es y el nombre del pais. (Tip utilizar if ternario y los 
    subindices del array para comparar y formar el template string)
`;
 */
console.log('Ejercicio _4 ->', campeon, final);
